...
if st.button('Show Recommendation'):
   ...
   # st.write(recommended_movie_names[['title', 'description']])
    ...
    # netflix_data['no_of_seasons']=netflix_data['no_of_seasons'].astype(str).astype(int)
    dic = {'TvShows':['Friends','rows','rachel','ram'],
    # ,"movie":["Lord of rings","rahul","ramesh","ketu"],
    'Duration':[10,20,30,40]}
    new_dic = pd.DataFrame(dic)
    new_dic = new_dic.set_index('TvShows')
    st.bar_chart(data = new_dic)
st.write('  '
         )
st.write(' ')
...
if EDA :
    st.write(
        "check out this [link](http://localhost:8888/notebooks/Desktop/ML%20Movie%20Recommand/Movie_Recommeder_system.ipynb)")